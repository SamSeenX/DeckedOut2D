...
export function updateUI(gameState, player) {
  ...
  if (gameState.hasArtifact) {
    const iconFile = gameState.targetArtifactItem
      ? gameState.targetArtifactItem.icon
      : null;
    if (iconFile) {
    ...
    artIcon.title = gameState.targetArtifactItem
      ? gameState.targetArtifactItem.name
      : "Artifact";
  } else {
    artIcon.textContent = "?";
  ...
  if (now - lastCompassUpdate > COMPASS_UPDATE_INTERVAL) {
    ...
    if (target) {
      // Add 0.5 to point to center of target tile, not top-left corner
