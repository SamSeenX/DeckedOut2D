import {
  ...
  MAX_HAZE,
  HEARTBEAT_MIN_VOLUME,
  HEARTBEAT_MAX_VOLUME,
} from "../data/config.js";
...
export function playDing() {
  ...
  osc.frequency.exponentialRampToValueAtTime(1046.5, now + 0.1); // Sustain briefly
...
export function playGameOverSequence() {
  ...
  const notes = [
    ...
    { freq: 185.0, time: 0.3, dur: 0.4 }, // F#3 (Tritone down)
    { freq: 130.81, time: 0.6, dur: 0.6 }, // C3
    { freq: 98.0, time: 1.0, dur: 1.5 }, // G2 (Low final note)
  ];
...
export function startHeartbeatSystem(getHazeLevel) {
  ...
  const beat = () => {
    ...
    const nextInterval =
      HEARTBEAT_MAX_INTERVAL -
      ratio * (HEARTBEAT_MAX_INTERVAL - HEARTBEAT_MIN_INTERVAL);

    // Calculate Volume based on Haze
    // Higher Haze = Louder
    const volume =
      HEARTBEAT_MIN_VOLUME +
      ratio * (HEARTBEAT_MAX_VOLUME - HEARTBEAT_MIN_VOLUME);
...
export async function playJson(url) {
  ...
  if (config) {
    // Debug: Uncomment to trace sound playback
